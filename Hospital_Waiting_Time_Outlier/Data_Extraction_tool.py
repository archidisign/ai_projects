...
def get_primary_oncologist(DB):
    onc_cnx = mysql.connector.connect(user='root',password='therapist', database=DB)
    ...
    SELECT
        ...
        Doctor.LastName
    FROM Patient
        JOIN PatientDoctor on Patient.PatientSerNum = PatientDoctor.PatientSerNum
        JOIN Doctor on Doctor.DoctorSerNum = PatientDoctor.DoctorSerNum
    WHERE
        PatientDoctor.OncologistFlag = 1 AND PatientDoctor.PrimaryFlag = 1 ''')
...
def get_doctors(DB):
    doc_cnx = mysql.connector.connect(user='root',password='therapist', database=DB)
    ...
    FROM Task
        JOIN Attendee ON Task.ActivityInstanceAriaSer = Attendee.ActivityInstanceAriaSer
...
def query_database(DB):
    ...
    WHERE
        Appointment.AliasSerNum = 3 AND Appointment.Status!='Cancelled' AND Appointment.State!='Deleted'
    UNION ALL SELECT
        Patient.PatientSerNum,
        Diagnosis.DiagnosisCode,
        Priority.PriorityCode,
    ...
    UNION ALL SELECT
        ...
        Priority.DueDateTime
    FROM Document
        JOIN Patient ON Document.PatientSerNum=Patient.PatientSerNum
...
def add_nopriorityCT(DB,data):
    ...
    FROM Appointment
        JOIN Patient ON Appointment.PatientSerNum=Patient.PatientSerNum
        JOIN Alias ON Appointment.AliasSerNum = Alias.AliasSerNum
    WHERE Appointment.AliasSerNum = 3 AND Appointment.Status != 'Cancelled' AND ScheduledStartTime > '2012-01-01 00:00:00' AND Appointment.PrioritySerNum=0 AND Appointment.DiagnosisSerNum=0
    UNION ALL SELECT
        Patient.PatientSerNum,
        Appointment.DiagnosisSerNum,
        Priority.PriorityCode,
    ...
    FROM Appointment
        ...
        JOIN Priority ON Appointment.PrioritySerNum = Priority.PrioritySerNum
        JOIN Alias ON Appointment.AliasSerNum = Alias.AliasSerNum
    WHERE Appointment.AliasSerNum = 3  AND Appointment.Status != 'Cancelled' AND Appointment.DiagnosisSerNum=0  AND ScheduledStartTime > '2012-01-01 00:00:00'
    AND (Priority.PriorityCode='SGAS_P3' OR Priority.PriorityCode='SGAS_P4') ''')
...
def add_nopriorityTasks(DB,data):
    ...
    task_cnx=mysql.connector.connect(user='root',password='therapist', database=DB)
...
def add_nopriorityEOTNT(DB,data):
    new_data = []
    EOTNT_cnx=mysql.connector.connect(user='root',password='therapist', database=DB)
    ...
    FROM Task
        JOIN Patient ON Task.PatientSerNum=Patient.PatientSerNum
        JOIN Alias ON Task.AliasSerNum = Alias.AliasSerNum
    WHERE Task.AliasSerNum = 6 AND Task.Status != 'Cancelled' AND Task.State != 'Deleted' AND Task.CreationDate > '2012-01-00 00:00:00' AND Task.PrioritySerNum=0 AND Task.DiagnosisSerNum=0
    UNION ALL SELECT
        Patient.PatientSerNum,
        Task.DiagnosisSerNum,
        Priority.PriorityCode,
    ...
    FROM Task
        ...
        JOIN Priority ON Task.PrioritySerNum = Priority.PrioritySerNum
        JOIN Alias ON Task.AliasSerNum = Alias.AliasSerNum
    WHERE Task.AliasSerNum = 6  AND Task.Status != 'Cancelled' AND Task.State != 'Deleted' AND Task.DiagnosisSerNum=0  AND Task.CreationDate > '2012-01-00 00:00:00' AND (Priority.PriorityCode='SGAS_P3' OR Priority.PriorityCode='SGAS_P4')''')
...
def dosimetry(DB):
    d_cnx=mysql.connector.connect(user='root',password='therapist', database=DB)
    d_cur=d_cnx.cursor()
    ### Fetch data from database
