...
def build_parser():
    ...
    parser.add_argument('--style-path', type=str,
                        dest='style_path', help='file with style to use', 
                        metavar='STYLE_PATH', required=True)
   
    parser.add_argument('--file-name', type=str,
                        dest='file_name', help='name of file to be saved', 
                        metavar='FILE_NAME', required=True)
   
    parser.add_argument('--iterations-nbr', type=int,
                        dest='iter_nbr',help='number of iterations before output result',
                        metavar='ITER_NBR', default=200)

    return parser

def check_opts(opts):
    if opts.in_path is None:
        print('In path not found!')
    if opts.out_path is None:
        print('Out path not found!')
    if opts.style_path is None:
...
def main():
    ...
    content_filename = opts.in_path
    content_image = load_image(content_filename, max_size=None)
    style_filename = opts.style_path
    ...
    img = style_transfer(content_image=content_image, style_image=style_image, content_layer_ids=content_layer_ids, style_layer_ids=style_layer_ids, weight_content=1.5, weight_style=10.0, weight_denoise=0.3, num_iterations=opts.iter_nbr, step_size=10.0)
    
    # convert nparray to picture and then save the picture
    img = np.clip(img, 0.0, 255.0)
    img = img.astype(np.uint8)
    image = PIL.Image.fromarray(img)
    image.save(opts.out_path+'/'+opts.file_name+'.jpg','JPEG')
